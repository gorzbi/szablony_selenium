...
public class SzukajJuveTest extends Configuration {
    ...
    public void szukamyJuve() throws IOException {
        ...
        // v3 -> element łapany w screen (zdefiniowany w danym page) z folderem do którego ma być zapisany screen
         if (rezultatSzukania.isDisplayed()) {
            test01.pass("Udało się wyszukać", ScreenElement.getElementScreenshot(browser, element, goodScreen, "nazwaPlikuPNG"));// nazwa folderu jest w zmiennej a png sami teraz nazywamy
        }
        else {
            test01.fail("Nie udało się wyszukać", ScreenElement.getElementScreenshot(browser, element, evilScreen, "nazwaPlikuPNG"));
        }
